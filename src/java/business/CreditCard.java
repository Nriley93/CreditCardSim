...
//    Blueprint for a credit card account
    //Fields
    ...
    //constructor for new account number
    public CreditCard(String path) {   
        this.climit = 0;
        this.cbal = 0.0;
        this.path = path;
        cerrmsg = "";
    ...
    public CreditCard(int AccountNumber, String path) {  
        ...
        try {
            ...
            BufferedReader in = new BufferedReader(
                    new FileReader(path + "CC" + AccountNumber + ".txt"));
...
//    Set methods
//    validates input as well as calls methods to build saved txt files
    public void setCharge(double a, String d) {
        ...
        if (a > climit - cbal) {
            ...
            cmsg = "Credit Card Charge of " + a + " declined: must be positive value.";
            cerrmsg = writeLog(cmsg);
        } else {
            cbal += a;
            cerrmsg = writeStatus();
    ...
    public void setCreditIncrease(double r) {
        ...
        cmsg = "";
        if (r < 100) {
            cmsg = "Credit Increase of " + r +
                    " declined: minimum increase is $100.";
            cerrmsg = writeLog(cmsg);
        } else {
    ...
    public void setInterestCharge(double r) {
        ...
        if(r < 0) {
            cmsg = "Interest of " + r +
                    " declined: must be positive value.";
            cerrmsg = writeLog(cmsg);
        } else {
...
//    Writes the format for saved information
    private String writeLog(String desc) {
        String result = "";
        try {
            PrintWriter out = new PrintWriter(new FileWriter(
                    this.path + "CCL" + acctno + ".txt", true));
...
//            reading the saved text files
    ...
    public int getAccountId() {return acctno;}
    public double getOutstandingBal() {return cbal;}
    public double getCreditLimit() {return climit;}
    public double getAvailableCr() {return climit - cbal;}
    ...
    public String getErrorMessage() {return cerrmsg;}
    public String getActionMsg() {return cmsg;}
}